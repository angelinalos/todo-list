...
function createHeader() {
  const header = createElement("header", "flex flex-col gap-7 ")
  const btnAdd = createElement(
    "button",
    "text-gray-100 rounded-full p-3 shadow-md shadow-lime-500 animate-pulse",
    "Add"
  )
  const btnDelete = createElement(
    "button",
    "text-gray-100 rounded-full p-3 shadow-md shadow-fuchsia-500",
    "Delete All"
  )
  btnDelete.id = "delete"
  btnAdd.id = "add"
...
function createMain(tasks) {
  ...
  const input = createElement(
    "input",
    "rounded-full border-2 border-lime-500 self-center shadow-sm shadow-lime-500 bg-black text-gray-100 px-2 lg:px-6"
  )
  input.id = "enter"
  main.append(input)
  tasks.forEach((task) => {
...
function createCard(task) {
  ...
  const text = createElement(
    "span",
    "text-gray-100 text-sm break-words lg:break-normal",
    task.text
  )
  const date = createElement(
    "span",
    "rounded-lg mx-3 text-white text-xs absolute bottom-0 right-0 mb-7 lg:mb-11 text-blue-600/75",
    task.date
  )
  // ////// Active delete
  const btnDelete = createElement(
    "button",
    "font-light rounded-md p-1 m-2 shadow-sm shadow-slate-800 text-gray-100",
    "rm"
  )
